...
- **Authentication:** API Key in `app.js`
...
3. **Verify Gemini API Key** in `app.js`:
   ```javascript
...
├── app.js              # Core JavaScript logic + Gemini API integration
├── README.md           # This file
├── SECURITY.md         # Security & ethical guidelines
├── ARCHITECTURE.txt    # Technical architecture
└── requirements.txt    # Dependencies (CDN-based)
```
...
- ✅ No data storage (stateless except session)
- ✅ No user accounts or login
- ✅ No cookies or tracking
- ✅ All AI calls go to Google Gemini (check Google's privacy policy)
- ✅ API key exposure (mitigated in production via backend proxy)

### **For Production:**
- Move API key to backend environment variable
- Call your own backend endpoint instead of direct Gemini API
